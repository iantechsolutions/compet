...
export const pedidosRouter = createTRPCRouter({
    ...
    update: publicProcedure.input(z.object({Id:z.number(),FechaCreacion: z.number(),Fecha_de_aprobacion: z.number().optional(),Fecha_de_envio: z.number().optional(),Estado: z.string(),Cliente: z.number()})).mutation(async ({ ctx, input }) => {
      await db
        .update(pedidos)
        .set({
            Cliente: input.Cliente,
